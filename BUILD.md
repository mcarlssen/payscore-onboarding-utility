...
**What we’re building:** A small Ruby on Rails app for internal users (customer success) to import properties and units from a CSV. The CSV format is fixed (see example in repo). The tool should make it easy to:
...
**Scope:** Internal tool only. No authentication. UI can be simple and unpolished. We are not building a production-ready system; we are demonstrating thoughtful handling of data safety, duplication, and clear communication.

---

## 2. Decisions

These choices are locked in. Document them in the README as assumptions/tradeoffs.

| Decision | Choice | Rationale |
|----------|--------|-----------|
| **Empty Unit** | Rows with blank Unit create a **Property only**; no Unit record is created. | Assessment: “Units are created based on the rows.” A blank unit is not a unit; it represents a building with no unit breakdown. |
| **Property unique identifier** | **Building name + full address** `(building_name, street_address, city, state, zip_code)`. | The assessment says building name *can be* the unique identifier, not *must be*. Same building name in different cities (e.g. “Avenue Apartments” in Seattle vs Boston) must be different properties. Using name+address as the composite key removes ambiguity and eliminates the “same name, different city” auto-dedupe risk. |
...
| **Building name normalization** | **Do not** normalize or canonicalize building name for identity. Use exact match on the composite (name + address). | Normalizing building names could incorrectly merge two different buildings. Only address fields may be normalized in a future phase (e.g. “Ave” vs “Avenue”); building name stays as-is. |
...
- **Do** normalize only address fields if you add normalization later; leave building name as-is for identity.
